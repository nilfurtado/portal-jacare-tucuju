...
function getApiBase() {
  ...
  // Padrão: se está em localhost:8000, usa painel em localhost:3000
...
async function tentarBackend() {
  ...
  const url = `${base}/portal/bootstrap`;
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), 4_000);
  try {
    console.log('[data.js] Tentando fetch:', url);
    const r = await fetch(url, { signal: ctrl.signal });
    console.log('[data.js] Resposta recebida:', r.status);
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    return await r.json();
  } catch (err) {
    console.error('[data.js] Erro no fetch:', err.message, 'URL:', url);
    throw err;
  } finally {
    clearTimeout(t);
  }
...
async function fallbackJSONs() {
  ...
  const [noticias, enquetes, videos, classificados, meta, colunas, classCats, config] = await Promise.all([
    fetch('data/noticias.json' + cacheBust).then(r => r.json()).catch(() => []),
    fetch('data/enquetes.json' + cacheBust).then(r => r.json()).catch(() => []),
    fetch('data/videos.json' + cacheBust).then(r => r.json()).catch(() => []),
    fetch('data/classificados.json' + cacheBust).then(r => r.json()).catch(() => []),
    fetch('data/dados.json' + cacheBust).then(r => r.json()).catch(() => ({ categorias: [], municipios: [] })),
    fetch('data/colunas.json' + cacheBust).then(r => r.json()).catch(() => []),
    fetch('data/classificados-categorias.json' + cacheBust).then(r => r.json()).catch(() => []),
    fetch('data/config.json' + cacheBust).then(r => r.json()).catch(() => ({})),
  ]);
  return {
    noticias: noticias || [],
    enquetes: enquetes || [],
    ...
    colunas: colunas || [],
    categorias: (meta && meta.categorias) || [],
    municipios: (meta && meta.municipios) || [],
    config: config || {},
  };
}
...
// ---------- Filtro de Lixeira ----------
/** Remove notícias deletadas (com removidoEm) */
function ativas(noticias) {
  return noticias.filter(n => !n.removidoEm);
}

// ---------- Funções derivadas (mesma assinatura de antes) ----------

export const byCategoria = (cat, limit = Infinity) =>
  ativas(getCache().noticias).filter(n => n.categoria === cat).slice(0, limit);

export const byMunicipio = (mun, limit = Infinity) =>
  ativas(getCache().noticias).filter(n => n.municipio === mun).slice(0, limit);

export const bySlug = (slug) =>
  ativas(getCache().noticias).find(n => n.slug === slug);

export const destaques = (n = 4) =>
  ativas(getCache().noticias).filter(noticia => !noticia.colunista).slice(0, n);

export const maisLidas = (n = 5) =>
  [...ativas(getCache().noticias)].sort((a, b) => b.views - a.views).slice(0, n);

export const ultimas = (n = 6) =>
  [...ativas(getCache().noticias)].sort((a, b) => new Date(b.data) - new Date(a.data)).slice(0, n);

export const relacionadas = (noticia, n = 3) =>
  ativas(getCache().noticias)
...
export function search(query) {
  ...
  return ativas(getCache().noticias).filter(n =>
...
export function tempoRelativo(iso) {
  ...
  if (d < 30) return `há ${w} ${w === 1 ? 'semana' : 'semanas'}`;
