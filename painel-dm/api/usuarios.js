...
// Configurar multer para uploads de avatar
...
// ========== HISTÓRICO ==========
router.get('/:id/historico', requerePermissao('usuarios'), async (req, res) => {
  try {
    const historico = await store.read(`usuarios-historico-${req.params.id}`, []);
    res.json(historico);
  } catch {
    res.json([]);
  }
...
// ========== AVATAR UPLOAD ==========
router.post('/:id/avatar', requerePermissao('usuarios'), upload.single('foto'), async (req, res) => {
  if (!req.file) {
  ...
  try {
    ...
    await store.update('usuarios', lista => {
      ...
      // Registrar histórico
      store.update(`usuarios-historico-${req.params.id}`, hist => [
        ...hist,
        {
          campo: 'foto',
          oldValue: prev.foto || '(nenhuma)',
          newValue: fotoUrl,
